...
GetDistances <- function(counts){
  ...
  distances <- c(0, startbase[2:num.nonzero.exons] - 
                   ...
                   1000000000000 * (as.numeric(chromosome[2:num.nonzero.exons]) - 
                                      as.numeric(chromosome[1:(num.nonzero.exons - 1)]))) #RT modified by adding as.numeric()
...
EstimateVariance <- function(counts, ref.sample.names, sample.weights){
  library(Hmisc)
  counts$var <- apply(counts[, ref.sample.names], 1, wtd.var, sample.weights, normwt=T) #error is here
  set.seed(1)
...
PrintCNVs <- function(test.sample.name, viterbi.state, 
  ...
  if (num.deletions == 0 & num.duplications == 0){
    df <- data.frame(SAMPLE=character(0), CNV=character(0), INTERVAL=character(0), 
                     ...
                     MID_BP=numeric(), TARGETS=character(0), NUM_TARG=numeric(0), Q_SOME=numeric(0), Q_NO=numeric(0), MLCN=numeric(0))
    return(df)
  }
  if (num.deletions > 0 & num.duplications > 0){
...
CallCNVs <- function(sample.name, counts, covariance, log_file, p=1e-08, Tnum=6, D=70000, numrefs=30, get.dfs=F, homdel.mean=0.2){
  if (!sample.name %in% names(counts)){stop("No column for sample ", sample.name, " in counts matrix")}
  ...
  #  counts$chromosome <- as.character(counts$chromosome) #RT modifided
  #}
  ######################################################################################################
  ...
  write.table(paste('[',format(Sys.time()),'] [',sample.name,'] if there are not too many CNVs, calculate the Q_SOME',sep=''), file=log_file, col.names=FALSE, append=TRUE)
  cat(paste('[',format(Sys.time()),']',' if there are not too many CNVs, calculate the Q_SOME\n',sep=''))
  if (nrow(cnvs) > 0 & nrow(cnvs) <= 50){
    qualities <- GenotypeCNVs(cnvs, sample.name, counts, covariance, log_file, p, Tnum, D, numrefs, 
                          emission.probs=emission.probs, 
                          distances=distances)
    for (i in 1:nrow(cnvs)){
      cnvs$Q_SOME[i] <- ifelse(cnvs$CNV[i]=="DEL", qualities[i, "SQDel"], 
                               qualities[i, "SQDup"])
      cnvs$Q_NO[i] <- ifelse(cnvs$CNV[i]=="DEL", qualities[i, "NQDel"], 
                               qualities[i, "NQDup"])
  ...
  # cnvs <- paste(format(Sys.time()),distances)
  write.table(paste('[',format(Sys.time()),'] [',sample.name,'] Done. return cnvs back.',sep=''), file=log_file, col.names=FALSE, append=TRUE)
  return(cnvs)
}
...
GenotypeCNVs <- function(xcnvs, sample.name, counts, covariance, log_file, p=1e-08, Tnum=6, 
                    D=70000, numrefs=30,
                    emission.probs=NULL, 
                    distances=NULL){
  if (!sample.name %in% names(counts)){stop("No column for sample ", sample.name, " in counts matrix")}
  ...
  if (is.null(emission.probs)){
    l <- CallCNVs(sample.name, counts, covariance, log_file, p, Tnum=6, D=70000, numrefs=30, get.dfs=T)

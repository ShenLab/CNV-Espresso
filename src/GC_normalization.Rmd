...
output_file_name <- paste(output_path,'canoes_calls_',batch_name,'.csv',sep='')
...
```
```{r}
RC_ref_weighted_df <- cbind(RC_afterNorm[,c(1:5)],counts$mean,RC_afterNorm$SP0000203)
head(RC_ref_weighted_df)
plot(RC_ref_weighted_df$gc,RC_ref_weighted_df$`RC_afterNorm$SP0000203`/RC_ref_weighted_df$`counts$mean`,ylim = c(0,5))
```

```{r}
range(RC_ref_weighted_df$`counts$mean`)
head(RC_ref_weighted_df)
loessMod50_afterNorm <- loess(counts$mean ~ gc, data=RC_ref_weighted_df, span=0.50) # 50% smoothing span
# get smoothed output
